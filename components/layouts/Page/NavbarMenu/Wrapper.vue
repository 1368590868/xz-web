  ...
  <template v-else-if="menu.children && menu.children.length > 0">
    <HeadlessPopover v-slot="{ open }">
      <HeadlessPopoverButton
        class="flex items-center transition-all duration-300 text-white text-[16px]  dark:text-gray-100">
        <span :class="[isActive ? 'font-bold text-[#E7900E]' : '']">{{
          parseMenuTitle(menu.title)
        }}</span>
        <Icon name="carbon:chevron-down" class="ml-1" :class="[open ? 'transform rotate-180' : '']" />
      </HeadlessPopoverButton>
      <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="translate-y-1 opacity-0"
        enter-to-class="translate-y-0 opacity-100" leave-active-class="transition duration-150 ease-in"
        leave-from-class="translate-y-0 opacity-100" leave-to-class="translate-y-1 opacity-0">
        <HeadlessPopoverPanel
          class="absolute  z-10 px-2 py-2 rounded-lg min-w-[150px] bg-[#000000]/[0.27] backdrop-filter backdrop-blur-md border-[#000000]/[0.1] dark:bg-gray-900 border dark:border-gray-600">
